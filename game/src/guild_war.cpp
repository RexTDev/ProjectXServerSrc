...
void CGuild::GuildWarPacket(DWORD dwOppGID, BYTE bWarType, BYTE bWarState)
{
	...
	int score = quest::CQuestManager::instance().GetEventFlagBR("score",g->GetName());
	BYTE minlevel = quest::CQuestManager::instance().GetEventFlagBR("minlevel",g->GetName());
	BYTE maxplayer = quest::CQuestManager::instance().GetEventFlagBR("maxplayer",g->GetName());
	BYTE horseriding = quest::CQuestManager::instance().GetEventFlagBR("horseriding",g->GetName());
#endif
	TPacketGCGuild pack;
...
#ifdef ENABLE_GUILD_WAR_EXTENSION
	pack2.bScore  = score;
	pack2.bMinLevel = minlevel;
	pack2.bMaxPlayer = maxplayer;
	pack2.bHorseRiding = horseriding;
#endif
...
#ifdef ENABLE_GUILD_WAR_EXTENSION
	sys_err("burdayim hehe");
	quest::CQuestManager::instance().RequestSetEventFlagBR("score", opp_g->GetName(), score);
	quest::CQuestManager::instance().RequestSetEventFlagBR("minlevel", opp_g->GetName(), minlevel);
	quest::CQuestManager::instance().RequestSetEventFlagBR("maxplayer", opp_g->GetName(), maxplayer);
	quest::CQuestManager::instance().RequestSetEventFlagBR("horseriding", opp_g->GetName(), horseriding);

	sys_err("buda ikinci hehe");
