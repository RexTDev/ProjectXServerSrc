...
#ifdef ENABLE_IP_BINDING
	int _AesEncrypt(lua_State* L)
	{
		std::string sStr = lua_tostring(L, 1);
		std::string sKey = lua_tostring(L, 2);
		std::string sIv = lua_tostring(L, 3);
		char buf_str[1024] = {0};
		{
			// copy
			strncpy(buf_str, sStr.c_str(), sizeof(buf_str));
			CryptoPP::CFB_Mode<CryptoPP::AES>::Encryption aes((const BYTE*)sKey.c_str(), 16, (const BYTE*)sIv.c_str());
			aes.ProcessData((BYTE*)buf_str, (BYTE*)buf_str, strnlen(buf_str, sizeof(buf_str))+1);
		}
		lua_pushstring(L, buf_str);
		return 1;
	}
	int _AesDecrypt(lua_State* L)
	{
		std::string sStr = lua_tostring(L, 1);
		std::string sKey = lua_tostring(L, 2);
		std::string sIv = lua_tostring(L, 3);
		char buf_str[1024] = {0};
		{
			// copy
			strncpy(buf_str, sStr.c_str(), sizeof(buf_str));
			CryptoPP::CFB_Mode<CryptoPP::AES>::Decryption aes((const BYTE*)sKey.c_str(), 16, (const BYTE*)sIv.c_str());
			aes.ProcessData((BYTE*)buf_str, (BYTE*)buf_str, strnlen(buf_str, sizeof(buf_str))+1);
		}
		lua_pushstring(L, buf_str);
		return 1;
