...
void CHARACTER::OpenMyShop(const char * c_pszSign, TShopItemTable * pTable, BYTE bItemCount)
{
	...
	for (BYTE i = 0; i < bItemCount; ++i)
	{
		...
		if (pkItem)
		{
			...
			{
				ChatPacket(CHAT_TYPE_INFO, LC_TEXT( "You can't sell this item in private shop because is binded!"));
				return;
			}
#endif
...
#if defined(ENABLE_WOLFMAN_CHARACTER) || defined(ENABLE_KEYTO_CHARACTER)
...
void CHARACTER::SetPlayerProto(const TPlayerTable * t)
{
	...
	//GM일때 보호모드
	if (!test_server
#ifdef ENABLE_GM_FLAG_IF_TEST_SERVER
	|| test_server
#endif
	)
	{
...
#ifdef FULL_YANG
void CHARACTER::ChangeGold(long long amount,bool unused)
{
...
void CHARACTER::SetGold(long long gold)
{
	m_points.gold = gold;
}

void CHARACTER::SendGold(bool isLogin)
{
	if (!GetDesc())
		return;
...
void CHARACTER::PointChange(BYTE type, int amount, bool bAmount, bool bBroadcast)
{
	...
	switch (type)
	{
		...
		case POINT_LEVEL:
			...
			if (GetJob() ==
#if defined(ENABLE_WOLFMAN_CHARACTER) && defined(ENABLE_KEYTO_CHARACTER)
							JOB_WOLFMAN || JOB_ELFE
#elif defined(ENABLE_WOLFMAN_CHARACTER) && !defined(ENABLE_KEYTO_CHARACTER)
							JOB_WOLFMAN
#elif !defined(ENABLE_WOLFMAN_CHARACTER) && defined(ENABLE_KEYTO_CHARACTER)
							JOB_ELFE
#endif
			...
			switch (val)
			{
				...
				case 90:	case 100:	case 110:	case 120:
					char szNoticeLevelUp[QUERY_MAX_LEN];
					snprintf(szNoticeLevelUp, sizeof(szNoticeLevelUp), "<Information> [CH%d]: [%s] erreichte Level [%u]. Gratulation!", g_bChannel, GetName(), GetLevel()); 
...
void CHARACTER::LoadSafebox(int iSize, DWORD dwGold, int iItemCount, TPlayerItem * pItems)
{
	...
	if (!bLoaded)
	{
		for (int i = 0; i < iItemCount; ++i, ++pItems)
		{
			...
			LPITEM item = ITEM_MANAGER::instance().CreateItem(pItems->vnum, pItems->count, pItems->id,false, -1, false
#ifdef ENABLE_SOULBIND_SYSTEM
			,pItems->bind
#endif
...
void CHARACTER::SetBlockMode(BYTE bFlag)
{
	...
	SetQuestFlag("game_option.block_duell_mode", bFlag & BLOCK_DUELL_MODE ? 1 : 0);
#endif
...
{
#ifdef ENABLE_WOLFMAN_CHARACTER || defined(ENABLE_KEYTO_CHARACTER)
...
void CHARACTER::DetermineDropMetinStone()
{
	...
	static const DWORD c_adwMetin[] =
	{
		...
		28012,
#endif
#if defined(ENABLE_KEYTO_CHARACTER) && defined(USE_ELFE_STONES)
		28013,
#endif
		28030,
		28031,
...
#else
		ChatPacket(CHAT_TYPE_COMMAND, "shop_add %d %d %s %d %d %d %d %d", it->second.shop_id, it->second.shop_vid, it->second.szSign, it->second.gold, it->second.item_count, it->second.rest_count, it->second.days, it->second.date_close);
#endif
		it++;
	}
}
...
const char* CHARACTER::RetLC_(const char* format, ...)
{
	char dest[1024 * 16];
	va_list argptr;
...
BYTE CHARACTER::GetLanguage()
{
	char szQuery[QUERY_MAX_LEN];
	snprintf(szQuery,sizeof(szQuery),"SELECT lang FROM account.account WHERE id = %u", GetAID());
	std::auto_ptr<SQLMsg> pMsg(DBManager::instance().DirectQuery(szQuery));
	BYTE lang = 0;
	if(pMsg->Get()->uiNumRows > 0)
	{
		MYSQL_ROW row = mysql_fetch_row(pMsg->Get()->pSQLResult);
		str_to_number(lang,row[0]);
	}
	return lang;
}

void CHARACTER::SetLanguage(BYTE lang)
{
	char szQuery[QUERY_MAX_LEN];
	snprintf(szQuery,sizeof(szQuery),"UPDATE account.account SET lang = %d WHERE id = %u", lang, GetAID());
	ChatPacket(7, "%s", szQuery);	// Lang Change Chat
	std::auto_ptr<SQLMsg> pMsg(DBManager::instance().DirectQuery(szQuery));
}
#endif
