...
void Acce_open(LPCHARACTER ch)
{
	...
	if (ch->IsAcceOpen())
	{
		ch->ChatPacket(CHAT_TYPE_INFO, LC_TEXT("The Acce creation window is already opened."));
		return;
	}
...
bool Acce_make(LPCHARACTER ch)
{
	...
	if (!(ch)->IsAcceOpen())
	{
		(ch)->ChatPacket(CHAT_TYPE_INFO, LC_TEXT("The Acce creation window is not open."));
		return false;
	}
...
#ifdef FULL_YANG
		ch->ChangeGold(-(acce_proto->gold), false);
#else
	...
	else
	{
		...
		ch->ChatPacket(CHAT_TYPE_INFO, LC_TEXT("Failed!"));
...
bool Acce_absorption_make(LPCHARACTER ch)
{
	...
	{
		(ch)->ChatPacket(CHAT_TYPE_INFO, LC_TEXT("You can use a sash to absorb the bonus just from weapons or armours."));
		return false;
	}
	else if (items[0]->GetAcceAttributeCount() > 0)
	{
		(ch)->ChatPacket(CHAT_TYPE_INFO, LC_TEXT("You must remove the bonus from sash before continue."));
		return false;
	}

#ifdef ENABLE_SOULBIND_SYSTEM
	if (items[1]->IsBind() || items[1]->IsUntilBind())
	{
		(ch)->ChatPacket(CHAT_TYPE_INFO, LC_TEXT("You can't absorp this item, because is binded!"));
		return false;
	}
#endif
	...
	ch->ChatPacket(CHAT_TYPE_INFO, LC_TEXT("The absorption process had success!"));
