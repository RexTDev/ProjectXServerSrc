...
#ifdef PLAYER_BLOCK_POLYMORPHED_ATTACK
	if (!IsGM() && IsPolymorphed() && pkVictim->IsPC())
	{	
...
#ifdef ENABLE_FIX_BUFF_WO_SWORD
	if (pAttacker && this)
	{
						...
						if (m_me->IsPC())
						{
							...
							switch (pkArrow->GetVnum())
							{
								...
								case ITEM_QUIVER_30_DAYS:
									iDam = CalcArrowDamage(m_me, pkVictim, pkBow, pkArrow, false);
									break;
								default:
