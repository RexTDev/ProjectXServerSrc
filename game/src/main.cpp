...
int main(int argc, char **argv)
{
	...
	std::ifstream exppet_table_open("/usr/home/game/share/locale/germany/pet_exp_table.txt");
	...
	while (!exppet_table_open.eof())
	{
		...
		{
			sys_log(0, "Pet EXP wurde geladen von: exppettable.txt: %d !", exppet_table_common[exppet_table_counter]);
			exppet_table_counter++;
		}
		else
		{
			fprintf(stderr, "[main] Pet-Exp Tabelle konnte nicht richtig geladen werden\n");
			break;
		}
	}
#endif
