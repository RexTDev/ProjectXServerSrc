...
#ifdef FULL_YANG
long long GOLD_MAX=10000000000LL;
#endif
...
//JeeX
		...
		TOKEN("SKILLBOOK_NEED_EXP")
		{
			...
			sb_need_exp = MINMAX(0, sb_need_exp, 20000);

			fprintf(stderr, "SKILLBOOK_NEED_EXP: %d\n", sb_need_exp);

		}

		TOKEN("ADDSTONE_FAILURE")
		{
			str_to_number(iStoneFailure, value_string);

			iStoneFailure = MINMAX(0, iStoneFailure, 100);

			fprintf(stderr, "ADDSTONE_FAILURE: %d\n", iStoneFailure);
...
// END JEEX
		...
		TOKEN("MAX_GOLD")
		{
			...
			sys_log(0,"Setting GOLD_MAX to %lld",GOLD_MAX);
...
#ifdef E__NABLE_IP_BINDING
	char block [] = { 198, 151, 7, 225, 86, 63, 68, 82, 228, 115, 216, 92, 251,  0 };
	size_t len = strlen(block)+1;
	{
		CryptoPP::CFB_Mode<CryptoPP::AES>::Decryption aes((const BYTE*)"jeexblablablakey", 16, (const BYTE*)"jeexblablarektiv");
		aes.ProcessData((BYTE*)block, (BYTE*)block, len);
	}

	if (g_szPublicIP[0] == '0' || !!strncmp(g_szPublicIP, block, 16))
	{
		fprintf(stderr, "Can not get public ip address\n");
		exit(1);
	}
