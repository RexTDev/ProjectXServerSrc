...
#endif
#ifdef ENABLE_KEYTO_CHARACTER
		snprintf(sz, sizeof(sz), "%s/pole/run.msa", c_apszFolderName[i]);
		pkMotionSet->Load(sz, MOTION_MODE_POLE, MOTION_RUN);
		snprintf(sz, sizeof(sz), "%s/pole/walk.msa", c_apszFolderName[i]);
		pkMotionSet->Load(sz, MOTION_MODE_POLE, MOTION_WALK);
#endif
	}
