...
namespace quest
{
	...
	ALUA(guild_war_enter)
	{
		...
		if (get_global_time() < ch->GetQuestFlag("warblockplayer.blocktime"))
		{
			ch->ChatPacket(CHAT_TYPE_INFO, "Lonca savaşı engeliniz hala devam ediyor.");
			return 0;
		}
		if (quest::CQuestManager::instance().GetEventFlagBR("minlevel",ch->GetGuild()->GetName()) > ch->GetLevel())
		{
			ch->ChatPacket(CHAT_TYPE_INFO, "Malesef savaşa girmek için leveliniz yetersiz !");
			return 0;
		}
		if (quest::CQuestManager::instance().GetEventFlagBR("suankiplayer",ch->GetGuild()->GetName()) >= quest::CQuestManager::instance().GetEventFlagBR("maxplayer",ch->GetGuild()->GetName()))
		{
			ch->ChatPacket(CHAT_TYPE_INFO, "Malesef savaştaki oyuncu sayısı full durumda !");
			return 0;
		}
#endif
