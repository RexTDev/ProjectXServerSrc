...
void CInputMain::BraveRequestPetName(LPCHARACTER ch, const char* c_pData)
{
	...
	if (ch->CountSpecifyItem(vid) > 0 && check_name(p->petname) != 0) {
		...
		ch->ChatPacket(CHAT_TYPE_INFO, "[PetSystem] - Falscher Name");
	}
}
#endif
...
void CInputMain::Exchange(LPCHARACTER ch, const char * data)
{
	...
	switch (pinfo->sub_header)
	{
		...
		case EXCHANGE_SUBHEADER_CG_ELK_ADD:	// arg1 == amount of gold
			...
			{
#ifdef FULL_YANG
				long long nTotalGold = ch->GetExchange()->GetCompany()->GetOwner()->GetGold() + pinfo->arg1;
...
void CInputMain::PartyParameter(LPCHARACTER ch, const char * c_pData)
{
	...
	if (ch->GetParty() && ch->GetParty()->GetLeaderPID() == ch->GetPlayerID())	// Fix: EXP-Verteilungeinstellung nur fur den Gruppenleader
