...
int CalcAttBonus(LPCHARACTER pkAttacker, LPCHARACTER pkVictim, int iAtk)
{
	...
	else if (pkVictim->IsPC())
	{
		...
		switch (pkVictim->GetJob())
		{
			...
			case JOB_ELFE:
				iAtk += (iAtk * pkAttacker->GetPoint(POINT_ATTBONUS_ELFE)) / 100;
				break;
#endif
		}
	}
...
#ifdef ENABLE_KEYTO_CHARACTER
			case JOB_ELFE:
				iAtk -= (iAtk * pkVictim->GetPoint(POINT_RESIST_ELFE)) / 100;
				break;
#endif
		}
	}
...
#ifdef ENABLE_KEYTO_CHARACTER
			case WEAPON_POLE:
				iDam = iDam * (100 - pkVictim->GetPoint(POINT_RESIST_POLE)) / 100;
