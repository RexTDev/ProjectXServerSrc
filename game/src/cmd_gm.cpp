...
#endif
#ifdef ENABLE_KEYTO_CHARACTER
			,tch->GetPoint(POINT_RESIST_ELFE)
#endif
	);
...
#endif
#ifdef ENABLE_KEYTO_CHARACTER
			,tch->GetPoint(POINT_RESIST_POLE)
#endif
	);
...
#endif
#ifdef ENABLE_KEYTO_CHARACTER
			,tch->GetPoint(POINT_ATTBONUS_ELFE)
#endif
	);
...
const struct set_struct
{
	...
	{ "race",		NUMBER,	"0. Warrior, 1. Ninja, 2. Sura, 3. Shaman, 4. Lycan, 5. Elfe"		},
...
#elif !defined(ENABLE_WOLFMAN_CHARACTER) && defined(ENABLE_KEYTO_CHARACTER)
	{ "race",		NUMBER,	"0. Warrior, 1. Ninja, 2. Sura, 3. Shaman, 4. Elfe"		},
#else
...
#ifdef PLAYER_BLOCK_SET_COMMAND
	if (tch->IsGM() == false)
	{
...
#ifdef ENABLE_KEYTO_CHARACTER
					case JOB_ELFE:
						dwRace = (mySex==SEX_MALE)?MAIN_RACE_ELFE_W:MAIN_RACE_ELFE_W;
						break;
#endif
				}
		...
		case DoSetTypes::WON:	// won
			{
				...
				DBManager::instance().SendMoneyLog(MONEY_LOG_MISC, 3, won);
...
#ifdef PLAYER_BLOCK_SETSKILLOTHER_COMMAND
	if (tch->IsGM() == false)
	{
...
ACMD (do_item_full_set)
{
	...
	case JOB_ELFE:
		{
			item = ITEM_MANAGER::instance().CreateItem(10309);
			if (!item || !item->EquipTo(ch, item->FindEquipCell(ch)))
				M2_DESTROY_ITEM(item);
			item = ITEM_MANAGER::instance().CreateItem(13149);
			if (!item || !item->EquipTo(ch, item->FindEquipCell(ch)))
			...
			if (!item || !item->EquipTo(ch, item->FindEquipCell(ch)))
				M2_DESTROY_ITEM(item);
			item = ITEM_MANAGER::instance().CreateItem(789);
			if (!item || !item->EquipTo(ch, item->FindEquipCell(ch)))
				M2_DESTROY_ITEM(item);
/*
			item = ITEM_MANAGER::instance().CreateItem(00000);
			if (!item || !item->EquipTo(ch, item->FindEquipCell(ch)))
				M2_DESTROY_ITEM(item);
*/
			item = ITEM_MANAGER::instance().CreateItem(14209);
