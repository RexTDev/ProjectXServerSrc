...
bool CHARACTER::UseItemEx(LPITEM item, TItemPos DestCell)
{
	...
	if (item->GetVnum() == 55001)
	{
		...
		if (pmsg2->Get()->uiNumRows > 0)
		{
			...
			if (atoi(row[0]) > 0)
			{
				...
				{
					ChatPacket(CHAT_TYPE_INFO, "Du musst zuerst dein Pet wegschicken.");
					return false;
				}

				DBManager::instance().DirectQuery("UPDATE new_petsystem SET duration =(tduration) WHERE id = %d", item2->GetID());
				ChatPacket(CHAT_TYPE_INFO, "Die Lebensenergie des Pet wurde um +50% erh?t.");
			}
			else
			{
				DBManager::instance().DirectQuery("UPDATE new_petsystem SET duration =(tduration/2) WHERE id = %d", item2->GetID());
				ChatPacket(CHAT_TYPE_INFO, "Die Lebensenergie deines Pets wurde wiederhergestellt.");
			}
			item->SetCount(item->GetCount() - 1);
			return true;
		}
	...
	if (item->GetVnum() >= 55701 && item->GetVnum() <= 55706)
	{
		...
		if (item2 = GetItem(DestCell))
		{
			if (item2->GetVnum() == 55002)
			{
				...
				{
					ChatPacket(CHAT_TYPE_INFO, "In der Transportbox befindet sich bereits ein Pet");
				}
#ifdef ENABLE_SOULBIND_SYSTEM
				if (item->IsBind() || item->IsUntilBind())
				{
					ChatPacket(CHAT_TYPE_INFO, "You can't put your Pet into the Box, because is binded!");
					return false;
				}
#endif
				else
	...
	if (item->GetVnum() == 55002 && item->GetAttributeValue(0) > 0)
	{
		...
		if(pos == -1)
		{
			ChatPacket(CHAT_TYPE_INFO, "Du hast nicht genugend Platz.");
			return false;
		}
...
#ifdef GM_BLOCK_SHOP_CREATION_SEIDENBUENDEL
										if (IsGM() == true)
										{
								...
								case USE_ADD_ACCESSORY_SOCKET:
									{
										...
										if (item2->IsAccessoryForSocket())
										{
											if (item2->GetAccessorySocketMaxGrade() < ITEM_ACCESSORY_SOCKET_MAX_NUM)
											{
												...
												if (number(1, 100) <= iStoneFailure)	// 50%
#else
												if (1)
#endif
												{
		...
		case ITEM_METIN:
			{
				...
				for (i = 0; i < ITEM_SOCKET_MAX_NUM; ++i)
					if (item2->GetSocket(i) >= 1 && item2->GetSocket(i) <= 2 && item2->GetSocket(i) >= item->GetValue(2))
					{
						// 석 확률
#ifdef ENABLE_ADDSTONE_FAILURE
						if (number(1, 100) <= iStoneFailure)	// 30% Succes Rate
#else
						if (1)
#endif
						{
...
bool CHARACTER::DestroyItem(TItemPos Cell)
{
	...
	if (item->IsBind() || item->IsUntilBind())
	{
		ChatPacket(CHAT_TYPE_INFO, "You can't destroy this item because is binded!");
		return false;
	}
#endif
					...
					for (WORD i = BELT_INVENTORY_SLOT_START; i < BELT_INVENTORY_SLOT_END; ++i)
						{
							...
							if (item2->GetVnum() == item->GetVnum())
							{
								...
								if (bCount == 0)
								{
									ChatPacket(CHAT_TYPE_INFO, LC_TEXT("¾ÆAIAU E¹μæ: %s"), item2->GetName());
									M2_DESTROY_ITEM(item);
...
#else
								ChatPacket(CHAT_TYPE_COMMAND, "PickUpItem %s", SSTR(item->GetVnum()).c_str());
#endif
								M2_DESTROY_ITEM(item);
								if (item2->GetType() == ITEM_QUEST)
									quest::CQuestManager::instance().PickupItem (GetPlayerID(), item2);
				...
				if (item->IsStackable() && !IS_SET(item->GetAntiFlag(), ITEM_ANTIFLAG_STACK))
				{
					...
					for (int i = 0; i < INVENTORY_MAX_NUM; ++i)
					{
						...
						if (item2->GetVnum() == item->GetVnum())
						{
							...
							if (bCount == 0)
							{
								owner->ChatPacket(CHAT_TYPE_INFO, LC_TEXT("¾ÆAIAU E¹μæ: %s ´OA¸·IºIAI %s"), GetName(), item2->GetName());
								ChatPacket(CHAT_TYPE_INFO, LC_TEXT("¾ÆAIAU Au´Þ: %s ´O¿¡°O %s"), owner->GetName(), item2->GetName());
								M2_DESTROY_ITEM(item);
								if (item2->GetType() == ITEM_QUEST)
...
#else
				owner->ChatPacket(CHAT_TYPE_COMMAND, "PickUpItemNew %s %s", GetName(), SSTR(item->GetVnum()).c_str());
#endif
...
bool CHARACTER::EquipItem(LPITEM item, int iCandidateCell)
{
	...
	if (true == item->IsEquipped())
	{
		...
		else if (true == CItemVnumHelper::Is_SPEED_BOOTS(dwVnum))
		{
			this->EffectPacket(SE_EQUIP_BOOTS);
		}
#endif
#ifdef ENABLE_NEW_HM_EFFECT
		else if (true == CItemVnumHelper::Is_Heromedal(dwVnum))
		{
			this->EffectPacket(SE_EQUIP_HEROMEDAL);
		}
#endif
#ifdef ENABLE_NEW_CHOC_EFFECT
		else if (true == CItemVnumHelper::Is_Chocolate_Amulet(dwVnum))
		{
			this->EffectPacket(SE_EQUIP_CHOCOLATE_AMULET);
		}
#endif
#ifdef ENABLE_EMOTION_MASK_EFFECT
		else if (true == CItemVnumHelper::Is_Emotion_Mask(dwVnum))
		{
...
bool CHARACTER::ItemProcess_Hair(LPITEM item, int iDestCell)
{
	...
	switch (GetJob())
	{
		...
		case JOB_ELFE:
		//	hair -= 0;  // If the char have a default hair don't change here.
			break;
#endif
		default :
			return false;
...
#ifdef ENABLE_KEYTO_CHARACTER
		case JOB_ELFE:
			if (item->GetAntiFlag() & ITEM_ANTIFLAG_ELFE)
				return false;
			break;
#endif
	}
...
#ifdef ENABLE_ITEM_EFFECT_SYSTEM
...
static bool SuraOnlySwords(DWORD dwVnum)
{
	int suraonly[] = {
		240,241,242,243,244,245,246,247,248,249,150,151,152,153,154,155,156,157,158,159,250,251,252,253,254,255,256,257,258,259,190,191,192,193,194,
		195,196,197,198,199,200,201,202,203,204,205,206,207,208,209,280,281,282,283,284,285,286,287,288,289,470,471,472,473,474,475,476,477,478,479
	};
	for(BYTE i =0;i<_countof(suraonly);++i)
	{
		if(suraonly[i]==dwVnum)
			return true;
	}
	return false;
}
bool CHARACTER::SameItem(LPITEM item1, LPITEM item2)
{
	...
	bool warriorFlags[2] = {IsAntiFlag(ITEM_ANTIFLAG_WARRIOR, table1->dwAntiFlags), IsAntiFlag(ITEM_ANTIFLAG_WARRIOR, table2->dwAntiFlags)};
	bool ninjaFlags[2] = {IsAntiFlag(ITEM_ANTIFLAG_ASSASSIN, table1->dwAntiFlags), IsAntiFlag(ITEM_ANTIFLAG_ASSASSIN, table2->dwAntiFlags)};
	bool suraFlags[2] = {IsAntiFlag(ITEM_ANTIFLAG_SURA, table1->dwAntiFlags), IsAntiFlag(ITEM_ANTIFLAG_SURA, table2->dwAntiFlags)};
	bool shamanFlags[2] = {IsAntiFlag(ITEM_ANTIFLAG_SHAMAN, table1->dwAntiFlags), IsAntiFlag(ITEM_ANTIFLAG_SHAMAN, table2->dwAntiFlags)};
#ifdef ENABLE_WOLFMAN_CHARACTER
	bool wolfmanFlags[2] = {IsAntiFlag(ITEM_ANTIFLAG_WOLFMAN, table1->dwAntiFlags), IsAntiFlag(ITEM_ANTIFLAG_WOLFMAN, table2->dwAntiFlags)};
#endif
#ifdef ENABLE_KEYTO_CHARACTER
	bool elfFlags[2] = {IsAntiFlag(ITEM_ANTIFLAG_ELFE, table1->dwAntiFlags), IsAntiFlag(ITEM_ANTIFLAG_ELFE, table2->dwAntiFlags)};
#endif
	if (ExcludeItems(item1->GetVnum()) || ExcludeItems(item2->GetVnum()))
		return false;
	int aflags1 = table1->dwAntiFlags;
	int aflags2 = table2->dwAntiFlags;
	if(item1->GetType() == ITEM_WEAPON && item2->GetType() == ITEM_WEAPON && item1->GetSubType() == WEAPON_BOW && item2->GetSubType() == WEAPON_BOW )
	{
		return true;
	}
	else if(item1->GetType() == ITEM_WEAPON && item2->GetType() == ITEM_WEAPON && item1->GetSubType() == WEAPON_TWO_HANDED && item2->GetSubType() == WEAPON_TWO_HANDED )
	{
		return true;
	}
	else if(item1->GetType() == ITEM_WEAPON && item2->GetType() == ITEM_WEAPON && item1->GetSubType() == WEAPON_DAGGER && item2->GetSubType() == WEAPON_DAGGER )
	{
		return true;
	}
#ifdef ENABLE_WOLFMAN_CHARACTER
	else if(item1->GetType() == ITEM_WEAPON && item2->GetType() == ITEM_WEAPON && item1->GetSubType() == WEAPON_CLAW && item2->GetSubType() == WEAPON_CLAW )
	{
		return true;
	}
#endif
	else if(SuraOnlySwords(item1->GetVnum())&&SuraOnlySwords(item2->GetVnum()))
	{
		return true;
	}
	else if (item1->GetType() == ITEM_WEAPON && item2->GetType() == ITEM_WEAPON && item1->GetSubType() == item2->GetSubType() 
...
#endif
	&& table1->bSize == table2->bSize && !SuraOnlySwords(item1->GetVnum()) && !SuraOnlySwords(item2->GetVnum())
	&& ((!warriorFlags[1] && !warriorFlags[0])
	|| (!ninjaFlags[1] && !ninjaFlags[0])
	|| (!suraFlags[1] && !suraFlags[0])
	|| (!shamanFlags[1] && !shamanFlags[0])
#ifdef ENABLE_WOLFMAN_CHARACTER
	|| (!wolfmanFlags[1] && !wolfmanFlags[0])
#endif
#ifdef ENABLE_KEYTO_CHARACTER
	|| (!elfFlags[1] && !elfFlags[0])
#endif
	))
		return true;
